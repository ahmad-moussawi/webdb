...
typedef struct {
    uint32_t page_id;      // Database Page ID currently focused (0xFFFFFFFF = arena cursor)
    uint16_t slot_idx;     // Cache slot index (0..1023) holding this page
    uint16_t cell_idx;     // Current slot directory index within the page (or arena row index)
    uint16_t cell_offset;  // Byte offset of the active row payload within the page
    uint8_t  btree_depth;  // B-tree traversal depth (0 = root/leaf)
    uint8_t  flags;        // Cursor status flags (0x1 = EOF, 0x2 = PINNED, 0x4 = ARENA_CURSOR)
...
typedef struct {
    ...
    uint32_t fault_page_id;   // Missing page ID during PAGE_FAULT               (offset 8)
    uint32_t result_count;    // Rows packed in current output chunk              (offset 12)
    uint32_t result_offset;   // Write offset in Output Result Buffer             (offset 16)
    uint32_t arena_offset;    // Allocation offset in Transient Query Arena       (offset 20)
    uint32_t rows_affected;   // Mutated/deleted row counter for DML              (offset 24..27)
    uint32_t reserved;        // Reserved for future flags (sort phase, agg mode) (offset 28..31)
    Cursor   cursors[16];     // 16 active cursors: table scans + index seeks     (offset 32..223,  192 bytes)
    Register registers[64];   // 64 evaluation registers (64 × 16B, 8B-aligned)   (offset 224..1247, 1024 bytes)
    uint8_t  _padding[32];    // Pad frame to exactly 1,280 bytes                 (offset 1248..1279)
} VmFrame;                    // Exact size: 1,280 bytes

typedef struct {
    uint8_t  depth;           // Current active nesting level: 0=outer, 1..7=subquery depth
    uint8_t  _pad[7];         // Alignment padding
    VmFrame  frames[8];       // 8 independent execution frames (8 × 1,280B = 10,240 bytes)
} VmContext;                  // Total size: 8 + 10,240 = 10,248 bytes (reserved region: 12,288 bytes)
```

**Nesting Semantics:**

- `vm_step()` always operates on `&ctx->frames[ctx->depth]` — the active frame.
- **Correlated subquery push:** `ctx->depth++`; reset `frames[ctx->depth].pc = 0` and clear its registers.
- **Subquery pop:** result register written into `frames[ctx->depth - 1]`; `ctx->depth--`; outer resumes at saved `pc`.
- **Sequential subqueries:** reuse the same frame slot (same `depth`); frame is reset between executions. No additional depth consumed.
- **Derived tables (`FROM (subquery)`):** materialized into the Query Arena; read back via an arena cursor (`flags = 0x4`). No frame push — depth stays at 0.

**Hard Limits (enforced at compile time):**

- Maximum correlated subquery nesting depth: **7** (`depth` 0..7, `frames[0..7]`). Exceeds → `SubqueryNestingTooDeepError`.
- Maximum registers per frame: **64** (`r[0]..r[63]`). Exceeds → `TooManyRegistersError`.
- Maximum cursor slots per frame: **16** (`c[0]..c[15]`). Supports up to 16-table joins per query level. Exceeds → `TooManyCursorsError`.
...
- **`next_page_id / right_child_page_id` (4 bytes, offset 6, Little-Endian):**
  - _For Leaf Pages (`0x0D`, `0x0A`):_ `next_page_id` pointer to the next sequential leaf sibling (enables linear scans without tree re-traversal).
  - _For Interior Pages (`0x05`, `0x02`):_ `right_child_page_id` pointer to the rightmost child subtree (where keys $>$ all keys on this page).
  - _For Free Pages (`0x00`):_ `next_free_page_id` pointer to the next page in the recycled LIFO free list.
...
4. **Defragmentation Trigger (On-Demand Page Compaction):**
   ...
   - The engine triggers an in-place **Page Compaction**:
     1. Uses the pre-allocated 4KB **Page Scratchpad** (`0x41C080..0x41D07F`) in shared memory (zero dynamic heap allocation).
...
while (currPageId !== 0) {
  ...
  if (pageType !== 0x00) {
    throw new CorruptPageError(
      currPageId,
      `Expected free page (0x00), got 0x${pageType.toString(16)}`,
    );
  }
...
|  `32..35`   | `next_catalog_page_id`       |  `uint32_t`   | Forward-compatible pointer to next chained table catalog page (`0` in V1; allocated when $> 16$ tables)                                          |
|  `36..39`   | `next_index_catalog_page_id` |  `uint32_t`   | Forward-compatible pointer to chained index catalog page (`0` in V1; allocated when $> 8$ indexes)                                               |
|  `40..99`   | `reserved`                   | `uint8_t[60]` | Zero-filled reserved space for future checkpoints, flags, and encryption parameters                                                              |

### 6.2 Master Table Catalog, Index Catalog & Dedicated Column Pages (Bytes 100..4095)

Supports up to 16 pre-allocated tables and 8 pre-allocated index descriptors on Page 1 (and $\infty$ via `next_catalog_page_id` / `next_index_catalog_page_id`), each table supporting up to **256 columns** with **64-character identifiers**:
...
typedef struct {
    uint16_t table_id;              // Numeric table identifier (1..65535)                    (offset 0..1,   2B)
    uint16_t column_count;          // Number of columns defined in this table (1..256)       (offset 2..3,   2B)
    uint32_t root_page_id;          // Table B+Tree root Page ID                              (offset 4..7,   4B)
    uint32_t col_catalog_page_id;   // First 4KB Catalog Page ID storing ColumnMeta records   (offset 8..11,  4B)
...
typedef struct {
    ...
    uint8_t  column_count;          // Number of indexed columns (1=single, 2..8=composite)  (offset 8,      1B)
    ...
    char     name[64];              // Index identifier name (null-padded UTF-8, max 64 chars)(offset 34..97,64B)
...
typedef struct {
    uint8_t  page_type;            // 0x0C (CATALOG_PAGE)                                    (offset 0,   1B)
    uint8_t  flags;                // Status flags                                           (offset 1,   1B)
    uint16_t col_count_in_page;    // Columns stored on this page (1..56)                    (offset 2..3, 2B)
    uint16_t table_id;              // Owning table ID                                        (offset 4..5, 2B)
    uint16_t start_col_index;      // Starting column index on this page (0, 56)             (offset 6..7, 2B)
    uint32_t next_col_catalog_page_id; // Pointer to next column catalog page if > 56 columns (offset 8..11, 4B)
    uint32_t page_checksum;            // CRC32 checksum across 4,096 bytes                  (offset 12..15,4B)
} CatalogPageHeader;                   // Exact size: 16 bytes
```

- **Page 1 Layout Partition:**
  ...
  - `bytes 2148..3171`: 8 pre-allocated `IndexDescriptor` slots ($8 \times 128\text{ B} = 1,024\text{ bytes}$).
  - `bytes 3172..3175`: `next_descriptor_catalog_page_id` (`uint32_t`, default `0`): Forward-compatible chained catalog page pointer for new catalog descriptor types (e.g., sequences, check constraints, triggers) if added in future versions.
  - `bytes 3176..4095`: 920 bytes reserved space on Page 1 for future catalog descriptors.
- **Dedicated Column Catalog Pages (`page_type = 0x0C`):**
  - Payload per 4KB catalog page: $4,096 - 16 = 4,080$ bytes.
  - Columns per page: $\lfloor 4080 / 72 \rfloor = 56$ columns.
...
- **Torn-Write Self-Healing on Startup:** If a crash or power cut occurs mid-write of Page 1 to the `.db` file, the `.wal` file remains non-empty and intact with the committed Page 1 frame (protected by its own CRC32 checksum). Startup recovery replays the committed Page 1 frame into `.db` before any read operations, cleanly self-healing the database.
  _(See [Phase 4: Transactions & WAL](./04_transactions_acid_wal.md) for the exhaustive Failure Analysis Matrix)._
...
- [ ] **Dynamic Null-Bitmap Alignment:** Verify bitwise null-checking for tables with 1, 8, 9, 16, 64, 100, and 256 columns (rejecting > 256 with `TooManyColumnsError`) without offset drift.
