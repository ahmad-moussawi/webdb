...
* **Query API:**
  ...
  db.from('events').where(jsonExtract('payload', '$.user.address.city'), '=', 'Berlin')
  ```
...
Unlike traditional databases where encryption requires invasive changes across row formats, B-tree balancing, and buffer pools, WebDB isolates storage entirely behind the **`IVfsAdapter` boundary** ([01_storage_memory_arch.md §7](file:///Users/ahmad/h/webdb/docs/plans/01_storage_memory_arch.md#L576)):
...
* **Salt Storage:** A 16-byte cryptographically secure random salt (`crypto.getRandomValues(new Uint8Array(16))`) is generated at database creation and stored in Page 1 reserved bytes `36..51` ([01_storage_memory_arch.md §6.1](file:///Users/ahmad/h/webdb/docs/plans/01_storage_memory_arch.md#L457)).
